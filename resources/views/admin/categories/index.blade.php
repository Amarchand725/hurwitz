                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>@if(!empty($value->title))
                      <a href="{{route('admin.sub-categories.index', ['category_id' => $value->id ])}}" class="nav-link" target="_blank">{{ucfirst($value->title)}} ( {{$value->sub_categories->count()}} )</a>
                      @else N/A @endif
                    </td>
                    ...
                    <td>
                      ...
                      <form id="destroyForm_{{$value->id}}" action="{{ route('admin.categories.destroy', $value->id) }}" method="POST" class="d-none">
                        @csrf
                        @method('delete')
                      </form>
                    </td>
